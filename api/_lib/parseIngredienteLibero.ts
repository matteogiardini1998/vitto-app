 ...
 * espone `recipeIngredient` in JSON-LD, es. "200 g di farina 00") in
 * quantità/unità/nome. Solo per il percorso JSON-LD/microdata: il percorso
 * AI (testo libero, foto) chiede già al modello di restituire questi tre
 * campi separati, molto più affidabile su input davvero disordinati.
 *
...
const NUMERI_PAROLA: Record<string, number> = { un: 1, uno: 1, una: 1, un_: 1, due: 2, tre: 3, quattro: 4, cinque: 5 };

const UNITA_NOTE = new Set(["q.b.", "qb", "pizzico", "pizzichi", "manciata", "manciate"]);
...
const REGEX_QUANTITA_UNITA = new RegExp(
  `^\\s*(?:(\\d+(?:[.,]\\d+)?)(?:\\s*[-–a]\\s*\\d+(?:[.,]\\d+)?)?|(\\d+)\\s*\\/\\s*(\\d+)|(un|uno|una|due|tre|quattro|cinque))\\s+(${UNITA_REGEX})\\b\\.?\\s*(?:di\\s+|d['’])?(.*)$`,
  "i",
);
...
// "una cipolla", "due uova": un numero in parola senza unità esplicita, l'unità è "un pezzo".
const REGEX_NUMERO_PAROLA = new RegExp(`^(${Object.keys(NUMERI_PAROLA).filter((p) => p !== "un_").join("|")})\\s+(.+)$`, "i");
...
export function parsaIngredienteLibero(riga: string): IngredienteParsato {
  const testo = riga.trim().replace(/\s+/g, " ");
